...
3~6장도 실습 하나마다 빈 작업 폴더를 사용합니다. 예를 들어 `doit-work/03-meeting`, `04-rename`, `05-monthly-report`, `06-candidates`처럼 구분하세요. 각 장 README에서 선택한 실습의 `start` 파일만 복사합니다. **4장의 코드 실행은 예외로 [4장 안내](../chapter04/README.md)에 따라 저장소 최상위를 열고 `work/ch04-xx`에 연습 복사본을 준비합니다.** 4장 명령은 저장소 최상위 터미널에서 실행합니다. 답안은 결과를 확인할 때 별도로 열어 보세요. 파일 이름 변경·분류 실습은 원본 보관 폴더 대신 연습용 복사본에서 진행합니다.
