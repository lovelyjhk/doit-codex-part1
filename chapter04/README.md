...
1. 저장소를 내려받아 압축을 풉니다.
2. 파일 처리·메일 실습(01~04)은 VS Code의 [파일] → [폴더 열기]로 `doit-codex-part1` 폴더를 엽니다. 각 안내에서 지정한 `start`를 **새 `work/ch04-xx` 하위 폴더에 복사**합니다. 명령은 저장소 최상위 터미널에서 실행합니다.
3. 스킬 만들기·재사용 실습(05~06)은 [meeting_skill_practice](05_create_skill/start/meeting_skill_practice/)를 저장소 밖의 새 연습 위치에 복사하고 그 폴더를 엽니다. 앞 실습에서 만든 폴더와 스킬을 다음 실습에서도 이어서 사용합니다.
...
5. 파일 처리·메일 프롬프트에서는 `이 폴더` 대신 `work/ch04-01/png`처럼 정확한 복사본 경로를 적습니다. `@` 뒤에 파일명을 입력했다면 제안 목록에서 해당 파일을 선택합니다.
