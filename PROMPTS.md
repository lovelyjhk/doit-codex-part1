# 바로 사용할 프롬프트

각 링크를 열면 복사할 문장이 코드 블록으로 들어 있습니다. 안내에 적힌 시작 파일만 별도 작업 폴더로 복사한 뒤 입력하세요.

| 책 위치 | 프롬프트와 사용 순서 |
|---|---|
| 01-1 파일을 읽고 실행하기 | [CSV 금액 합계 코드 작성과 실행](chapter01/examples/file-task.md) |
| 01-2 프롬프트 6원칙 | [교정·요약·매출·회의록·단계별 발표 구성](chapter01/examples/prompts.md) |
| 01-3 반복 규칙 | [AGENTS.md 읽기](chapter01/templates/README.md) |
| 01-3 재사용 템플릿 | [문서 요약·오류 해결·데이터 요약](chapter01/templates/prompts.md) |
| 1장 문제 1 | [완성 프롬프트 3개](solutions/ch01/ex01_1.md) |
| 1장 문제 2 | [변수형 템플릿과 사용 예](solutions/ch01/ex01_2.md) |
| 1장 문제 3 | [규칙 읽기 요청](chapter01/problems/ex01_3.md) |
| 02-1 설치 확인 | [현재 작업 폴더 확인](chapter02/01_setup/README.md) |
| 02-2 첫 명령 | [hello.py 생성과 실행](chapter02/02_first_command/README.md) |
| 02-3 같은 스레드 | [이름·날짜 출력과 후속 수정](chapter02/03_thread_memory/README.md) |
| 02-3 새 스레드 메모리 | [달빛책방 설정과 새 대화 요청](chapter02/03_thread_memory/memory.md) |
| 02-3 달빛책방 확장 예시 | [노래 3곡·책·음료로 오늘의 추천 만들기](chapter02/03_thread_memory/today-picks.md) |
| 02-3 파일 태그 | [README.md 참고 요청](chapter02/04_file_tag/README.md) |
| 02-3 diff | [인사말 변경·재수정·적용 전 제안](chapter02/05_diff/README.md) |
| 02-2 한 걸음 더 | [작업 영향 검토](chapter02/command_check/README.md) |
| 2장 문제 1 | [파일 태그 유무 비교](ch03/problems/ex03_2.md) |
| 2장 문제 2 | [같은 파일 생성 요청으로 승인 방식 비교](ch03/problems/ex03_3.md) |

`[입력 파일]` 같은 대괄호가 있는 재사용 템플릿은 실제 값으로 바꿉니다. 그 밖의 실습 문장은 안내대로 파일을 준비하면 그대로 사용할 수 있습니다.
