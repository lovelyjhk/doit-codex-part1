# 1장 코덱스란 무엇인가

1. [본문 예시와 입력 파일](examples/README.md)을 살펴봅니다.
2. [파일을 읽고 실행하는 예제](examples/file-task.md)에서 요청과 결과를 확인합니다.
3. [프롬프트 관리](templates/README.md)에서 반복 규칙과 템플릿을 준비합니다.
4. 마무리 문제를 풀고 답안과 비교합니다.

| 마무리 문제 | 모범 답안 |
|---|---|
| [1 부족한 프롬프트 고치기](problems/ex01_1.md) | [답안](../solutions/ch01/ex01_1.md) |
| [2 내 업무용 템플릿 만들기](problems/ex01_2.md) | [답안](../solutions/ch01/ex01_2.md) |
| [3 반복 규칙 파일 초안 작성](problems/ex01_3.md) | [답안](../solutions/ch01/ex01_3.md) |
